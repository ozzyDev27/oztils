def nonIndexedRange(rangeInp: int): return list(range(1,int(rangeInp)+1))
...
def remove(strIn, toRemove):
	...
	return strIn
def fileAppend(file: str,toWrite: str):
	import os
	localOpen = open(file, "a")
	localOpen.write(str(toWrite))
	localOpen.close()
def fileErase(file: str):
	import os
	localOpen = open(file, "w")
	localOpen.write("")
	localOpen.close()
def fileDelete(file: str):
	import os
	os.remove(file)
def fileRename(file: str,toRename: str):
	import os
...
def isPrime(n: int):
	...
	return True
def deleteAllOf(ls: list,target):
	while target in ls:
